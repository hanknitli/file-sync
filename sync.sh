...
dry_run(){
    ...
    rsync --dry-run -avPz --delete --human-readable --out-format="%o: %M: %f" "${source}/" "${dest}/" || exit_code=$?
...
sync_exec(){
	...
	touch "${LOG_FILE}"
	rsync -avPz --delete --human-readable --log-file=${LOG_FILE} "${source}/" "${dest}/" || exit_code=$?
